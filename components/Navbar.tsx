...
export default function Navbar() {
  ...
  useEffect(() => {
    fetch("/api/categories")
      .then((r) => r.json())
      .then((d) => {
        if (d.statusCode === "SUCCESS") setCategories(d.data || []);
      })
      .catch(console.error);
  }, []);
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (!t.closest("#mobile-menu") && !t.closest("#mobile-menu-btn"))
        closeMobileMenu();
    };
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center h-14 md:h-16 gap-3">
            ...
            >
              ...
              <span className="text-[10px] text-white self-start mt-1">™</span>
            </Link>
            ...
            <div className="flex-1 px-4 py-4 space-y-1 text-gray-800">
              {user ? (
                <>
                  ...
                  <DrawerLink href="/account" onClick={closeMobileMenu}>
                    My Account
                  </DrawerLink>
                  <DrawerLink href="/track-order" onClick={closeMobileMenu}>
                    Track Order
                  </DrawerLink>
                  <DrawerLink href="/contact" onClick={closeMobileMenu}>
                    Contact Us
                  </DrawerLink>
                  <DrawerLink href="/stores" onClick={closeMobileMenu}>
                    Store Locator
                  </DrawerLink>
                  ...
                  {categories.length > 0 && (
                    <div className="pt-3 border-t border-gray-100">
                      ...
                      <div
                        className={`overflow-hidden transition-all duration-300 ${categoryOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}
                      >
                  ...
                  <div className="pt-3 border-t border-gray-100">
                    ...
                    <div
                      className={`overflow-hidden transition-all duration-300 ${genderOpen ? "max-h-48 opacity-100" : "max-h-0 opacity-0"}`}
                    >
                      <div className="pt-1 pb-2 pl-3 space-y-0.5">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 md:hidden bg-white border-t border-gray-200 flex items-center h-16 shadow-lg">
        <Link
          href="/"
          className={`flex-1 flex flex-col items-center justify-center gap-0.5 transition ${pathname === "/" ? "text-primary" : "text-gray-500"}`}
        >
          <Home size={20} />
        ...
        <Link
          href="/cart"
          className={`flex-1 flex flex-col items-center justify-center gap-0.5 relative transition ${pathname === "/cart" ? "text-primary" : "text-gray-500"}`}
        >
        ...
        <Link
          href="/wishlist"
          className={`flex-1 flex flex-col items-center justify-center gap-0.5 relative transition ${pathname === "/wishlist" ? "text-primary" : "text-gray-500"}`}
        >
          <Heart size={20} />
          {wishlistCount > 0 && (
            <span className="absolute -top-1 -right-1 bg-secondary text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center">
        ...
        <Link
          ...
          className={`flex-1 flex flex-col items-center justify-center gap-0.5 transition ${pathname === "/account" || pathname === "/auth/login" ? "text-primary" : "text-gray-500"}`}
        >
          <User size={20} />
